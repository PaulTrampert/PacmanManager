...
public class CliToolRunner(ICliOutputHandlerRegistry outputHandlerRegistry)
{
    ...
    /// <param name="outputHandler">Optional handler for the output of the tool.</param>
    ...
    public Task<int> RunToolAsync(ICliTool tool, ICliOutputHandler? outputHandler = null, CancellationToken ct = default)
    {
        var outputHandlers = outputHandler == null ? [] : (ICliOutputHandler[])[outputHandler];
        return RunToolAsync(tool, outputHandlers, ct);
    }
