...
public sealed class LibAlpm : ILibAlpm
{
    ...
    /// </exception>
    public IPackage LoadPackageFile(string path, bool full = true, int sigLevel = 0)
    {
        ...
        try
        {
            ...
            unsafe
            {
                IntPtr pkgHandle = IntPtr.Zero;
                int result = NativeMethods.alpm_pkg_load(
                    _handle,
                    (byte*)pathPtr.ToPointer(),
                    full ? 1 : 0,
                    sigLevel,
                    &pkgHandle);

                if (result != 0 || pkgHandle == IntPtr.Zero)
                {
                    AlpmErrno err = GetLastError();
                    throw new AlpmException(err, $"Failed to load package file '{path}': {GetErrorString(err)}");
                }

                return new AlpmPackage(pkgHandle, ownsHandle: true);
            }
        }
        finally
        {
